resource "azurerm_private_endpoint" "private_endpoint" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = coalesce(
      try(var.mssql_server_output["${each.value.private_service_connection.private_connection_resource_id}"].id, null),
      ...
      try(var.key_vault_managed_hardware_security_module_output["${each.value.private_service_connection.private_connection_resource_id}"].id, null),
      try(var.recovery_services_vault_output["${each.value.private_service_connection.private_connection_resource_id}"].id, null),
