resource "azurerm_cdn_frontdoor_rule_set" "main" {
  for_each = try({ for key, value in var.cdn_frontdoor_rule_sets_data : key => value }, {})
...
resource "azurerm_cdn_frontdoor_rule" "main" {
  ...
  cdn_frontdoor_rule_set_id = azurerm_cdn_frontdoor_rule_set.main[each.value.cdn_frontdoor_rule_set_name].id
