resource "azurerm_firewall" "firewall" {
  ...
  resource_group_name = coalesce(
    ...
    try(var.resource_group_output["${each.value.value.resource_group_name}"].name, null)
  )
  ...
  firewall_policy_id = coalesce(
    try(each.value.firewall_policy_id, null),
    try(var.firewall_policy_output["${each.value.firewall_policy_name}"].id, null)
  )
  ...
  dynamic "ip_configuration" {
    ...
    content {
      ...
      public_ip_address_id = try(coalesce(
        try(each.value.ip_configuration[ip_configuration.key].public_ip_address_id, null),
        try(var.public_ip_output["${each.value.ip_configuration[ip_configuration.key].public_ip_address_name}"].id, null),
      try(var.public_ip_output["${each.value.ip_configuration[ip_configuration.key].public_ip_address_key}"].id, null)), null)
      subnet_id = coalesce(
