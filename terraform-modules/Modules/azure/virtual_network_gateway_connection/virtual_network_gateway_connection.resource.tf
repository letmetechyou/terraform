...
resource "azurerm_virtual_network_gateway_connection" "virtual_network_gateway_connection" {
  ...
  authorization_key = each.value.type == "ExpressRoute" ? try(
    coalesce(
      try(each.value.authorization_key, null),
      try(var.express_route_circuit_authorization_output["${each.value.authorization_key_name}"].authorization_key, null),
      try(module.express_route_circuit_authorization.express_route_circuit_authorization_output_names["${each.value.authorization_key_name}"].authorization_key, null)
  ), null) : null #If in the same subscription then the value can be null since no authorization key is needed. 
