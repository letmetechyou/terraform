 ...
 */
  ...
  ip_forwarding_enabled          = each.value.network_configuration["${strrev(split("-", strrev("${each.key}"))[0])}"].ip_forwarding_enabled          #lookup(each.value.network_configuration,split("-",each.key)[1]).enable_ip_forwarding
  accelerated_networking_enabled = each.value.network_configuration["${strrev(split("-", strrev("${each.key}"))[0])}"].accelerated_networking_enabled #lookup(each.value.network_configuration,split("-",each.key)[1]).enable_ip_forwarding
