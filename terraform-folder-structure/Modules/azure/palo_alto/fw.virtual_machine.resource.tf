...
data "azurerm_key_vault_secret" "main" {
  ...
  { for i in range(v.qty) : "${v.name_prefix}${format("%02d", v.start + i)}" => v if v.build && v.existing_key_vault_secret_name != null }]...)
  name         = lower(each.value.existing_key_vault_secret_name)
...
resource "azurerm_virtual_machine" "virtual_machine" {
  ...
  os_profile {
    ...
    admin_password = try(data.azurerm_key_vault_secret.main[each.key].value, null)
 ...
 */
  ...
  lifecycle {
    ...
    ignore_changes  = [os_profile_linux_config, primary_network_interface_id, network_interface_ids]
  }
}
