resource "azurerm_storage_account" "main" {
  ...
  # Optional Arguments
  queue_encryption_key_type         = each.value.account_tier == "Premium" ? "Service" : each.value.queue_encryption_key_type
  sftp_enabled                      = each.value.sftp_enabled
  public_network_access_enabled     = each.value.public_network_access_enabled
  table_encryption_key_type         = each.value.account_tier == "Premium" ? "Service" : each.value.table_encryption_key_type
  ...
  dynamic "blob_properties" {
    ...
    for_each = each.value.account_tier == "Premium" ? [] : each.value.blob_properties != null ? [1] : []
    ...
    content {
      ...
      dynamic "delete_retention_policy" {
        ...
        for_each = each.value.blob_properties.delete_retention_policy != null && each.value.blob_properties.delete_retention_policy.enabled == true ? [1] : []
...
resource "azurerm_storage_account_customer_managed_key" "main" {
  for_each = { for key, value in var.storage_account_data : key => value if value.enabled && value.cmk_enabled }
  #for_each = { for key, value in var.storage_account_data : key => value if value.enabled }
